<template>
	<view class="signature" :class="{ landscape }">
		...
		<view class="signature-area" v-if="landscape">
			<l-signature disableScroll ref="signatureRef" :penColor="penColor" :penSize="penSize" :key="landscape"
				uid="0" :minLineWidth="2" :openSmooth="openSmooth" :boundingBox="bBox" landscape></l-signature>
		</view>
		<view class="signature-area" v-if="!landscape">
			<l-signature disableScroll ref="signatureRef" :penColor="penColor" :penSize="penSize" :key="!landscape"
				uid="1" :minLineWidth="2" :openSmooth="openSmooth" :boundingBox="bBox"></l-signature>
		</view>
		<view class="screen-handle" @tap="change">
			<!-- #ifdef APP-NVUE -->
		...
		<view class="tools">
			<view class="tools-item" @tap="isShowPen = !isShowPen;">
				...
				<!-- #endif -->
				<!-- #ifndef APP-NVUE -->
				<image class="tools-item__image"
					src="https://fastly.jsdelivr.net/gh/liangei/image@latest/brush-fill.svg" mode="widthFix"></image>
				<!-- #endif -->
			</view>
			<view class="tools-item" @tap="onClick('undo')">
				...
				<!-- #ifndef APP-NVUE -->
				<image class="tools-item__image"
					src="https://fastly.jsdelivr.net/gh/liangei/image@latest/reply-fill.svg" mode="widthFix"></image>
				<!-- #endif -->
			</view>
			...
			<view class="tools-item" @tap="onClick('clear')">
				...
				<!-- #ifndef APP-NVUE -->
				<image class="tools-item__image"
					src="https://fastly.jsdelivr.net/gh/liangei/image@latest/brush-2-fill.svg" mode="widthFix"></image>
				<!-- #endif -->
			</view>
			<view class="tools-item" @tap="onClick('save')">
				...
				<!-- #endif -->
				<!-- #ifndef APP-NVUE -->
				<image class="tools-item__image" src="https://fastly.jsdelivr.net/gh/liangei/image@latest/save-fill.svg"
					mode="widthFix"></image>
				<!-- #endif -->
			</view>
				...
				<view class="popup-item">
					<view class="popup-item__head">
						...
						<view class="popup-item__more">
							...
							<switch class="popup-item__more-switch" @change="() => openSmooth = !openSmooth"
								:checked="openSmooth" />
						</view>
					</view>
					<view class="popup-item__head">
		...
		<!-- <view class="buttons">
			<button @tap="onClick('clear')">清空</button>
			<button @tap="onClick('undo')">撤销</button>
			<button @tap="onClick('save')">保存</button>
			<button @tap="bBox = !bBox">bBox{{bBox?'开':'关'}}</button>
			<button @tap="onClick('openSmooth')">压感{{openSmooth?'开':'关'}}</button>
		</view> -->
