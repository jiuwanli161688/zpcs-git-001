    ...
    <view class="goods-list" v-if="type == 'twoColumns'">
      <view v-for="(item, index) in res" :key="index" class="goods-item">
        ...
        <view class="goods-detail">
          <div class="title clamp" v-html="lightSearchStr(keyword, item.goodsName)" @tap="navigateToDetailPage(item)">
          </div>
          ...
          <div class="store-seller-name" v-if="storeName" @tap="navigateToStoreDetailPage(item)">
            <div class="text-hidden">
              <u-tag style="margin-right: 10rpx" size="mini" mode="dark" v-if="item.selfOperated" text="中品严选"
                type="error" />
      ...
      <div v-for="(item, index) in res" :key="index" class="goods-row">
        <div class="flex goods-col">
          <div class="goods-img" @tap="navigateToDetailPage(item)">
            <u-image width="230rpx" border-radius="16" height="230rpx" mode="aspectFit"
              :src="item.goodsImage || item.thumbnail">
              <u-loading slot="loading"></u-loading>
            </u-image>
          </div>
          <div class="goods-detail">
            <div class="title clamp3" @tap="navigateToDetailPage(item)">
              {{ item.goodsName }}
            </div>
            <view class="price-box" @tap="navigateToDetailPage(item)">
              <div class="price" v-if="item.price != undefined">
