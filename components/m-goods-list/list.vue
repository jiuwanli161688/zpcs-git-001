<template>
	<view>
		<view class="box-null" v-if="res.length == 0">
			<empty-box text="暂无数据~" />
		</view>
		<!-- 一行两列商品展示 -->
		<view class="goods-list" v-if="type == 'twoColumns' && res.length > 0">
			<view v-for="(item, index) in res" :key="index" class="goods-item">
				<view class="image-wrapper" @tap="navigateToDetailPage(item)">
					<u-image :src="item.thumbnail" width="100%" height='330rpx' mode="aspectFit">
						<u-loading slot="loading"></u-loading>
					</u-image>
				</view>
				<view class="goods-detail">
					<div class="title clamp" v-html="lightSearchStr(keyword, item)"
						@tap="navigateToDetailPage(item)">
					</div>
					<view class="price-box" @tap="navigateToDetailPage(item)">
			...
			$options.filters.goodsFormatPrice(item.price)[1]
					...
					<div class="promotion" @tap="navigateToDetailPage(item)">
					...
					<div class="count-config" @tap="navigateToDetailPage(item)">
						<span>已售 {{ item.buyCount || "0" }}</span>
						<span>{{ item.commentNum || "0" }}条评价</span>
					</div>
					<!-- <div class="store-seller-name" v-if="storeName" @tap="navigateToStoreDetailPage(item)">
		...
		<!-- 一行一列商品展示 -->
		<div v-if="type == 'oneColumns' && res.length > 0" class="goods-one-row">
			<div v-for="(item, index) in res" :key="index" class="goods-row">
				<div class="flex goods-col">
					<div class="goods-img" @tap="navigateToDetailPage(item)">
					...
					<div class="goods-detail">
						<div class="title clamp3" @tap="navigateToDetailPage(item)">{{ item.goodsName }}</div>
						<view class="price-box" @tap="navigateToDetailPage(item)">
							<div class="price" v-if="item.price != undefined">
								¥<span>{{ $options.filters.goodsFormatPrice(item.price)[0] }} </span>.{{
			$options.filters.goodsFormatPrice(item.price)[1]
						...
						<div style="overflow: hidden" @tap="navigateToDetailPage(item)" class="count-config">
							<span style="float: left; font-size: 22rpx">已售 {{ item.buyCount || '0' }}</span>
							<span style="float: right; font-size: 22rpx">{{ item.commentNum || '0' }}条评价</span>
						</div>
						<!-- <div style="overflow: hidden" @tap="navigateToStoreDetailPage(item)" class="count-config">
...
export default {
	...
	methods: {
      ...
      let crtStr = ''
      if(str.selfOperated) {
        crtStr = `<span style="color: #fff;padding: 2px 5px;font-size: 10px;margin-right: 5px;border-radius: 10px;background:${this.lightColor}">中品严选</span>`
      }
      if (str.customized) {
        crtStr = `<span style="color: #fff;padding: 2px 5px;font-size: 10px;margin-right: 5px;border-radius: 10px;background:#2193e2">定制</span>`
      }
			if (!keyword) {
					crtStr = crtStr + `<span>${str.goodsName}</span>`
			} else {
				let unicodes = '';
				...
				crtStr = crtStr + (str.goodsName ? str.goodsName.replace(reg, matchValue =>
      ...
      return crtStr;
		...
		navigateToDetailPage(item) {
			...
			uni.navigateTo({
				url: `/pages/product/goods?id=${item.id}&goodsId=${item.goodsId}&source=${source}`,
			});
		},
		// 跳转地址
