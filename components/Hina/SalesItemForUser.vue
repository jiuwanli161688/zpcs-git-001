	...
	<view class="cpn">
      ...
      <view class="line">
		...
		<view class="ft">
			...
			<view class="link">
				<view class="link-item" v-if="sales.status == 2" @tap="handlerSelfExpress">自行邮寄</view>
				<view class="link-item" v-if="sales.status == 2" @tap="handlerDoorExpress">上门取件</view>
				<view class="link-item link-primary" @tap="cancelSales"
					v-if="statusText == '申请中' || statusText == '等待商品回寄'">
					取消售后</view>
...
export default {
	...
	methods: {
		// 触发上门取件
		handlerDoorExpress: function () {
			this.$emit('door', { index: this.$props.index })
		},
		// 触发自行邮寄
		handlerSelfExpress: function () {
			this.$emit('self', { index: this.$props.index })
		},
		// 触发售后详情
