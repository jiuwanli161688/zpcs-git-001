						...
						<view class="goods-price"
							...
							<span v-if="goodsDetail.promotionPrice && !pointDetail">
								...
								<span class="goods-price-promotionShow goods-price-bigshow">{{
									$options.filters.goodsFormatPrice(goodsDetail.promotionPrice)[0] }}</span>
								.{{ $options.filters.goodsFormatPrice(goodsDetail.promotionPrice)[1] }}
							</span>
							<span v-if="pointDetail.points">
								<span class="goods-price-promotionShow goods-price-bigshow">{{ pointDetail.points
								}}</span>
								积分
							</span>
							<div class="promotion-box">
								...
								<span class="goods-price-bigshow">{{
									$options.filters.goodsFormatPrice(goodsDetail.price)[0] }}</span>
				...
				<scroll-view class="goods-skus-box" :scroll-y="true">
					...
					<view class="goods-skus-view" :key="specIndex" v-for="(spec, specIndex) in formatList">
						<view class="skus-view-list">
							...
							<!-- 正常逻辑 循环出sku -->
							<view v-if="!parentOrder" :class="{ active: spec_val.value == currentSelected[specIndex] }"
								class="skus-view-item" v-for="(spec_val, spec_index) in spec.values" :key="spec_index"
								@tap="handleClickSpec(spec, specIndex, spec_val, currentSelected[specIndex])">
								{{ spec_val.value }}
							</view>

							<!-- 拼团购买，仅筛选出当前拼团类型商品 -->
							<view v-if="parentOrder && spec_val.skuId == goodsDetail.id"
								:class="{ active: spec_val.value == currentSelected[specIndex] }" class="skus-view-item"
					...
					<view class="goods-skus-number flex flex-a-c flex-j-sb">
						...
						<uni-number-box class="uNumber" :min="1" :max="goodsDetail.quantity" v-model="payNum"
							@valueNull="valueNull"></uni-number-box>
					</view>
				</scroll-view>
				<!-- 按钮 -->
				<view class="btns" v-if="isCart">
					<view class="box-btn card" @tap="changeCartSku">确定</view>
				</view>
				<view class="btns" v-else>
					<view class="box-btn card"
						v-if="buyType != 'PINTUAN' && goodsDetail.goodsType != 'VIRTUAL_GOODS' && goodsDetail.customized === false && isFull"
						@tap="addToCartOrBuy('cart')">加入购物车</view>
					<view class="box-btn buy" @tap="addToCartOrBuy('buy')">立即购买</view>
				</view>
			</view>
		</u-popup>
	</div>
...
export default {
	...
	methods: {
		...
		handleClickSpec(val, index, specValue,) {
			let temp = this.currentSelected.find(i => i == specValue.value)
			if (!temp) {
				this.payNum = 1
			}
		...
		changeCartSku() {
			...
			API_trade.deleteSkuItem(this.oldGoodsSkuId).then((res) => {
				API_trade.addToCart(data).then(res => {
					if (res.data.code == 200) {
						this.$emit('getCardData');
						this.closeMask();
					}
				});
			})
			...
			if (val == 'cart') {
				...
				API_trade.addToCart(data).then(res => {
					if (res.data.code == 200) {
						uni.hideLoading();
						uni.navigateTo({
							url: `/pages/order/fillorder?way=${data.cartType}&addr=${this.addr.id || ''}${val == 'buy' ? `&customized=${this.goodsDetail.customized}` : ''}&parentOrder=${encodeURIComponent(JSON.stringify(this.parentOrder))}`
						});
						this.closeMask();
		...
		formatText() {
			...
			this.formatList.forEach(item => {
				this.currentSelected.forEach((val) => {
					item.values.forEach((specValue, i) => {
						if (val === specValue.value) {
							data.push(specValue.value)
						}
					})
				})
			})
			this.selectName = data.join()
		},
		valueNull(val) {
			if (!val) {
				this.payNum = val
