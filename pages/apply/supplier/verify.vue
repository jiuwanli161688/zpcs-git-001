<template>
    <view class="container" v-if="isComplete">
        ...
        <view>
            ...
            <view v-if="status != 'PAYMENT_FINISHED'" class="lnk-common lnk-primary" @tap="handlerBack">返回上一步，重新填写信息</view>
        </view>
    </view>
</template>
...
export default {
    ...
    methods: {
        ...
        handlerSure: function () {
            getVerifyPrice(2).then(res => {
                if (res.data.code == 200) {
                    if (res.data.result == 'PAYMENT_FINISHED') {
                        this.$u.toast('还未收到您的转账信息，请您确认已经转账,并耐心等待。', 3000)
                        return
                    }
                    if (res.data.result == 'WAIT_SIGN') {
                        uni.navigateTo({
        ...
        handlerBack: function () {
            ...
            // uni.navigateTo({
            //     url: '/pages/apply/supplier/archives?source=' + this.source
            // })
        }
    },
