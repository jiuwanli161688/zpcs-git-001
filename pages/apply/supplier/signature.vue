      ...
      <view class="min-photo">
        <view class="col-signature">
          <view class="rs-file" @tap="handlerUpfile">
            <image class="icon-signature" :src="iconSignature" mode="aspectFit" v-if="fileSignature.length == 0">
            </image>
            <image class="file-signature" :src="fileSignature" mode="aspectFit" v-if="fileSignature.length > 0"></image>
          </view>
        </view>
      </view>
...
import { distributorDetail, distributorUploadSigning, storePreviewProtocol } from "../../../api/distributor";
import { applySupplierFourth, applySupplierFourthEcho, renewApplySupplierFourth } from "../../../api/store";
...
export default {
  ...
  methods: {
      ...
      uni.chooseImage({
        ...
        success: (res) => {
          ...
          uni.uploadFile({
            ...
            name: "file",
            header: {
              accessToken: storage.getAccessToken() ? storage.getAccessToken() : this.token,
            },
            success: (uploadFileRes) => {
              ...
              if (data.code == 200) {
                ...
                this.$u.toast("上传失败，登录过期，请重新登录或重新扫码后再次上传。")
              }
    ...
    handlerSign: function () {
      ...
      uni.navigateTo({
        url: `/pages/apply/supplier/signing?access=${this.token}`
      })
    },
      ...
      if (!this.submitting) {
        ...
        if (this.valid()) {
          ...
          if (this.isRenewState) { // 续签
            renewApplySupplierFourth(this.objData).then(res => {
              uni.hideLoading();
              this.submitting = false;
              if (res.data.code == 200) {
                uni.redirectTo({
                  url: '/pages/apply/supplier/status'
                })
              } else {
            ...
            applySupplierFourth(this.objData).then(res => {
              uni.hideLoading();
              this.submitting = false;
              if (res.data.code == 200) {
                uni.redirectTo({
  ...
  onLoad: async function (options) {
    ...
    if (tempSearch.length > 0) {
      ...
      console.log(tempArrFilter, "tempArrFilter")
      if (tempArrFilter.length == 2) {
        storage.setAccessToken(tempArrFilter[0].slice(7))
        storage.setRefreshToken(tempArrFilter[1].slice(8))
    ...
    if (options.source == 'already') {
      uni.showLoading();
      applySupplierFourthEcho().then(res => {
        uni.hideLoading();
        if (res.data.code == 200) {
