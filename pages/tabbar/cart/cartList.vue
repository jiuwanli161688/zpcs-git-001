<template>
    <view class="wrapper" :style="{ backgroundImage: `url(${cartBg})` }">
        <view class="cart-container">
            <view class="cart-header">
                <text class="cart-title">购物车</text>
                <image class="cart-icon" :src="cartIcon"></image>
            </view>
            <view class="top-background" :style="{ backgroundImage: `url(${cartBg2})` }"></view>
            ...
            <view v-if="!loading && (cartDetail.cartList == '' || cartDetail.cartList == [] || !cartDetail)" class="empty">
                ...
                <view class="empty-tips">
                    空空如也
                    <navigator class="navigator" url="/pages/tabbar/home/index" open-type="reLaunch">随便逛逛></navigator>
                </view>
            </view>

            <!-- 店铺商品信息 -->
            <view class="content">
                <view class="box box2" :class="{ invalid: isInvalid(item) }" v-for="(item, index) in cartDetail.cartList" :key="index">
                    <view class="tab">
                        <view class="store-line">
                            ...
                            <span class="store-name wes store-line-desc">由【{{ item.storeName }}】供货</span>
                    ...
                    <u-swipe-action
                        :show="skuItem.selected"
                        :disabled="true"
                        @open="openAction(skuItem)"
                        :options="options"
                        bg-color="#fff"
                        ref="swiperAction"
                        class="cart-item"
                        v-for="(skuItem, i) in item.skuList"
                        :index="i"
                        :key="skuItem.goodsSku.id"
                        @click="changeActionTab(skuItem)"
                        @longpress="changeActionTab(skuItem)">
                        <!-- 满减活动 -->
                        <view v-if="Object.keys(skuItem.promotionMap).length != 0">
                            <view v-if="getPromotion(skuItem).includes('FULL_DISCOUNT')">
                                <view class="promotion-notice" v-if="item.promotionNotice">
                        ...
                        <view class="goods-row" :class="{ invalid: isInvalid(skuItem) }">
                                ...
                                <view>
                                    <u-checkbox-group>
                                        ...
                                        <u-checkbox
                                            ...
                                            :active-color="lightColor"
                                            class="c-left"
                                            :value="skuItem.checked"
                                            @change="checkboxChange(skuItem)"></u-checkbox>
                                        <!-- #endif -->
                                    </u-checkbox-group>
                                    <!--                <span class="invalid" v-else style="font-size: 24rpx">失效</span>-->
                                </view>
                                <u-image border-radius="10" :fade="true" @tap="navigateToGoods(skuItem)" width="160rpx" height="160rpx" :src="skuItem.goodsSku.thumbnail" />
                            </view>
                            <view class="goods-content">
                                ...
                                <!-- 规格 -->
                                <p class="sp-type" @tap="shutMask(skuItem)">{{ skuItem.goodsSku.simpleSpecs }}<u-icon name="arrow-right" color="#c0c4cc" size="20"></u-icon></p>
                                <p class="sp-type" v-if="skuItem.goodsSku.salesModel == 'WHOLESALE'">批发商品</p>
                                <p class="sp-number">
                                    <view class="sp-price">
                                        <!-- <view class="default-color" :class="{'main-color':Object.keys(skuItem.promotionMap).length ==0  }"> -->
                                    ...
                                    <view>
                                        <uni-number-box
                                            class="uNumber"
                                            :min="1"
                                            :max="getMaxQuantity(skuItem)"
                                            :disabled-plus="isMaxQuantityReached(skuItem)"
                                            :disabled="isInvalid(skuItem)"
                                    ...
                                    <!-- 如果当有促销并且促销是 限时抢购 -->
                                    <!-- promotions -->
                                    <view class="promotions-list" v-if="Object.keys(skuItem.promotionMap).length != 0">
                                        <view class="promotions-item-seckill" v-if="getPromotion(skuItem).includes('SECKILL')">
                                            距秒杀结束:
                                            <u-count-down
                                                show-border
                                                :hide-zero-day="true"
                                                ...
                                                border-color="#ededed"
                                                font-size="24"
                                                :timestamp="getCountDownTime(skuItem)">
                                            </u-count-down>
                                        </view>
                                    </view>
                                    ...
                                    <!-- <view class="priceDetail-flowPrice" :class="{'main-color':skuItem.priceDetailDTO}"
                  ...
                  <span>.{{ $options.filters.goodsFormatPrice(skuItem.priceDetailDTO.flowPrice)[1] }} </span>
                                    ...
                                    <view style="margin-left: 20rpx" v-if="!skuItem.checked && skuItem.errorMessage">
            ...
            <u-modal v-model="deleteShow" :confirm-style="{ color: lightColor }" @confirm="deleteConfirm" show-cancel-button :content="deleteContent" :async-close="true"></u-modal>
            <!-- 结账 -->
            <view class="box box6" :style="{ bottom: tabbarHeight + 'px' }">
                <view class="navL">
                    ...
                    <span class="price" v-if="!isEditMode">
                        <view class="prices">
                            <view class="fullPrice">
                                <span class="number" v-if="cartDetail && cartDetail.priceDetailDTO" style="font-size: 24rpx">
                                    合计:
                                    <span style="color: #e22221">¥</span>
                                    <span style="color: #e22221; font-size: 40rpx">{{ $options.filters.goodsFormatPrice(cartDetail.priceDetailDTO.flowPrice)[0] }}.</span
                                    ><span style="color: #e22221; font-size: 30rpx">{{ $options.filters.goodsFormatPrice(cartDetail.priceDetailDTO.flowPrice)[1] }}</span>
                                </span>
                                <span class="number" v-else>合计:0.00</span>
                            </view>
                            <view class="discountPrice">
                                <span class="number" v-if="cartDetail && cartDetail.priceDetailDTO && cartDetail.priceDetailDTO.payPoint">
                                    积分抵扣:
                                    <span>¥{{ $options.filters.goodsFormatPrice(cartDetail.priceDetailDTO.payPoint)[0] }}</span
                                    >.<span>{{ $options.filters.goodsFormatPrice(cartDetail.priceDetailDTO.payPoint)[1] }}</span>
                                </span>
                            </view>
                            <view
                ...
                <u-popup z-index="3" close mode="bottom" height="50%" closeable v-model="discountDetailsFlag" border-radius="20">
                    <view class="discount-list">
                        ...
                        <view class="discount-way">
                            <view class="discount-item" v-if="cartDetail.priceDetailDTO">
                                <span>商品总额</span>
            ...
            <popupGoods
                ...
                :isCart="true"
                :oldGoodsSkuId="oldGoodsSkuId"
                v-if="buyMask"
                :buyMask="buyMask" />
        </view>
        <!-- 自定义tabbar -->
    <custom-tabbar class="custom-tabbar" :current="2" ></custom-tabbar>
    </view>
</template>
<script>
...
export default {
    components: { uniNumberBox, popupGoods,CustomTabbar }, // 数量加减组件
    data() {
        return {
            loading: false,
            iconEmptyCart: `${this.iconCommon}/static/emptyCart.png`,
            cartBg: `${this.iconCommon}/static/cart/cart-bg.png`,
            cartBg2: `${this.iconCommon}/static/cart/cart-bg2.png`,
            cartIcon: `${this.iconCommon}/static/cart/cart-icon.png`,
    ...
    methods: {
        ...
        navigateToGoods(val) {
            uni.navigateTo({
                url: '/pages/product/goods?id=' + val.goodsSku.id + '&goodsId=' + val.goodsSku.goodsId + '&source=CARD_RECOMMEND'
            })
        },
         ...
         * 获取商品最大可购买数量
         */
        getMaxQuantity(skuItem) {
            // 获取库存数量，优先使用 quantity，如果没有则使用 stock
            const stock = skuItem.goodsSku.quantity !== undefined ? skuItem.goodsSku.quantity : skuItem.goodsSku.stock;

            // 如果当前数量已经大于等于库存，返回当前数量作为最大值
            if (skuItem.num >= stock) {
                return skuItem.num;
            }
         ...
         * 判断是否达到最大数量（用于禁用加号按钮）
         */
        isMaxQuantityReached(skuItem) {
            const stock = skuItem.goodsSku.quantity !== undefined ? skuItem.goodsSku.quantity : skuItem.goodsSku.stock;
            return skuItem.num >= stock;
        },
        ...
        numChange: debounce(function (val) {
            // 获取库存数量
            const stock = val.goodsSku.quantity !== undefined ? val.goodsSku.quantity : val.goodsSku.stock;

            // 如果输入的数量超过库存，自动调整为库存数量
            if (val.num > stock) {
                this.$nextTick(() => {
                    val.num = stock;
                });
                uni.showToast({
                    title: `库存不足，已调整为最大库存数量：${stock}`,
                    icon: 'none',
                    duration: 2000
                });
                return;
                    ...
                    .then((result) => {
                        ...
                        if (result.data.success) {
                            ...
                            for (let i = 0; i < this.cartDetail.cartList.length; i++) {
                                ...
                                // 判断该商家是否全选：只有当该商家下所有商品都被选中时，该商家才被标记为全选
                                if (storeTotalItems > 0 && storeSelectedItems === storeTotalItems) {
                                    item.checked = 1
                                } else {
                                    item.checked = 0
                                    checkOuted = false
                            ...
                            // 全局全选：只有当所有商家的所有商品都被选中时，全局全选按钮才被勾选
                            this.checkout = checkOuted && totalItems > 0 && (totalItems === selectedItems)
                            //如果商品数量为0，不能勾选全选
