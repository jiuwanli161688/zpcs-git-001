<template>
  <div class="layout">
    <u-sticky>
      <div class="goods-cell-title">
        <div class="goods-item-title" :class="{ 'selected-title': selected.index == index }"
          @tap="handleClickTitle(title, index)" v-for="(title, index) in res.list[0].titleWay" :key="index">
    ...
    <div class="goods-list">
      <div v-if="item.___index != undefined
        ? selected.index == item.___index
        : selected.val == item.type
        " @tap="handleClick(item)" class="goods-item" v-for="(item, item_index) in res.list[0].listWay"
        :key="item_index">
        <div class="goods-img">
      ...
      <div v-if="res.list[0].titleWay[selected.index].bindCategory && goodsData.length" v-for="(item, index) in goodsData"
        :key="index" class="goods-item" @tap="handleClick(item)">
        <div class="goods-img">
          <u-image :src="item.thumbnail" height="350rpx" mode="aspectFit" width="100%">
            <u-loading slot="loading"></u-loading>
          </u-image>
        </div>
        <div class="goods-desc">
