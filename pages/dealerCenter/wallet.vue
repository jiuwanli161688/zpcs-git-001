...
<!--		<view class="btn-balance" @tap="handlerCashOut">我要提现</view>-->
		<empty-box v-if="objData.isComplete && objData.arrData.length == 0" text="暂无交易流水" />
...
<script>
  ...
  import {getDistributorMoney, getStoreWallet} from "@/api/dealerCenter";
        ...
        getStoreWallet(this.params).then((res) => {
          if (res.data.success) {
			...
			handlerCashOut: function(){
				uni.navigateTo({
					url: '/pages/dealerCenter/operator'
				})
			},
			...
			handlerArrow: function(){
				uni.navigateTo({
					url: '/pages/dealerCenter/transaction'
				})
			},
		},
		onLoad: function(){
