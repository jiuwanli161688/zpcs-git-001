<template>
  <u-popup class="popup" v-model="addressFlag" :height="setup.height" :mode="setup.mode" :border-radius="setup.radius"
    @close="closeAddress()" closeable>
    <view class="header-title">选择地址</view>
    <view class="view-box" v-if="addressDetail">
      <view class="view-item" v-for="(item, index) in addressDetail" :key="index" @tap="clickAddress(item)">
        ...
        <view class="view-box-dress" :class="{ 'box-dress-blod': item.isDefault }">{{ item.consigneeAddressPath |
    clearStrComma }}</view>
      </view>
    </view>
    ...
    <view class="btns">
      <view class="box-btn light" @tap="getpicker">选择其他地址</view>
      <view class="box-btn" @tap="sureAddress()">确定</view>
    </view>
