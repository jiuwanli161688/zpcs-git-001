<template>
  <view class="evaluate-box">
    <view class="eva-section" @tap="toComment(goodsDetail.goodsId, goodsDetail.grade)">
      <view class="e-header">

        <view class="evaluate-title">评价</view>
        <text class="evaluate-num">{{ commDetail.total || '0' }}+</text>
        <text class="tip">好评率 {{ grade }}%</text>
      </view>
      <div v-if="commDetail && commDetail.records && commDetail.records.length > 0">
        <view class="eva-box" v-for="(commItem, commIndex) in commDetail.records.slice(0, 3)" :key="commIndex">
            ...
            <scroll-view scroll-x class="scroll-x" v-if="commItem.image">
              <view class="img">
                <u-image border-radius="12" class="commImg" width="160rpx" height="160rpx"
                  ...
                  @tap.stop="previewImg(commItem.image, index)"></u-image>
              </view>
            </scroll-view>
            <view class="bot">
...
<!--        <u-empty text="该商品暂无评价" mode="message"></u-empty>-->
      </div>
    </view>
    <!-- 查看全部评价按钮 -->
    <view v-if="commDetail && commDetail.records && commDetail.records.length > 0" class="eva-section-btn"
      @tap="toComment(goodsDetail.goodsId, goodsDetail.grade)">
      <text>查看全部评价</text>
    </view>
  </view>
</template>
