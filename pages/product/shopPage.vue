  ...
  <view>
    ...
    <div>
      <u-navbar :border-bottom="false">
        <u-search v-model="keyword" @search="search" @tap="search" @custom="search" placeholder="请输入搜索"></u-search>
      </u-navbar>
      <div class="wrapper">
        <!-- 店铺信息模块 -->
        <div class="store flex">
          ...
          <div class="box">
            <div class="store-name" @tap="getStoreLicencePhoto">
              {{ storeInfo.storeName || "" }}
          ...
          <div class="collection">
            <!--            <div class="collection-btn" @tap="whetherCollection">
              {{ isCollection ? "已关注" : "+ 关注" }}
            </div>-->
          </div>
        </div>
      ...
      >
        <view class="card-box" v-for="(item, index) in couponList" :key="index">
          <view class="card" @tap="getCoupon(item)">
            <view class="money">
              <view>
      ...
      <div v-if="basePageData">
        ...
        <div class="category" v-if="current == 1">
          <div class="category-item" v-for="(item, index) in categoryList" :key="index">
            <div class="flex" @tap="getCategoryGoodsList(item)">
            ...
            <div class="child-list" v-if="item.children && item.children.length != 0">
              <div class="child" @tap="getCategoryGoodsList(child)" :key='i' v-for="(child, i) in item.children">
                {{ child.labelName }}
              </div>
            </div>
          </div>
