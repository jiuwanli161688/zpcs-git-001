  ...
  <view class="wrapper" v-if="flag">
    ...
    <div>
      ...
      <div v-if="isMaster && !isOver">
        <div class="share-user" v-if="master.toBeGroupedNum" @tap="share()">
          邀请好友拼团
        </div>
        <div class="home" @tap="handleClickHome()">
          去首页逛逛
        </div>
      </div>
      <div v-if="!isMaster && !isOver && !isBuy">
        <div class="share-user" @tap="toBuy">
          参与拼团
        </div>
      </div>
      <div v-if="!isMaster && !isOver && isBuy">
    ...
    <shares @close="closeShare"
      ...
      type="pintuan" :thumbnail="data.promotionGoods.thumbnail" :goodsName="data.promotionGoods.goodsName"
      v-if="shareFlag" />
  </view>
</template>
