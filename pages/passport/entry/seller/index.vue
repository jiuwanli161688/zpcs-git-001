<template>
  <div class="wrapper">
    ...
    <div>
      ...
      <div class="step-list">
        <div class="step-item" :class="{ active: current == index }" v-for="(item, index) in entrySteps" :key="index">
          {{ item.title }}
        </div>
      </div>
      <div class="submit" @tap="keepOn()">开始填写</div>
      <div class="notice" @tap="getEntryNotice">查看店铺入驻协议</div>
    </div>
  </div>
</template>
