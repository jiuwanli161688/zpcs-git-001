		...
		<div v-if="!wechatLogin">
			...
			<div v-show="!enableUserPwdBox">
				<div v-show="current == 0">
					...
					<div :class="!enableFetchCode ? 'disable' : 'fetch'" @tap="fetchCode" class="btn">
						获取验证码
					</div>
				</div>
				<!-- 输入验证码 -->
			...
			<div v-show="enableUserPwdBox">
				...
				<u-input :custom-style="inputStyle" :placeholder-style="placeholderStyle" placeholder="请输入密码" class="mobile"
					focus v-model="userData.password" type="password" />

				<div :class="!enableUserBtnColor ? 'disable' : 'fetch'" @tap="passwordLogin" class="btn">
					帐号密码登录
				</div>
			</div>
			...
			<div class="flex" v-show="current != 1">
				...
				<div class="tips">
					未注册的手机号验证后将自动创建用户账号，登录即代表您已同意<span @tap="navigateToPrivacy('PRIVACY_POLICY')">《隐私协议》</span>
					<span @tap="navigateToPrivacy('USER_AGREEMENT')">
						《用户协议》
					</span>
				</div>
			</div>

			<div v-if="current != 1" class="user-password-tips" @tap="enableUserPwdBox = !enableUserPwdBox">
			...
			<div class="flex login-list">
				<div v-if="item.code" :style="{ background: item.color }" class="login-item"
					...
					<u-icon v-if="item.title != 'APPLE'" color="#fff" size="42" :name="item.icon"
						@tap="navigateLogin(item)">
					</u-icon>
					<u-image v-else src="/static/appleidButton@2x.png" :lazy-load="false" @tap="navigateLogin(item)"
						width="80" height="80" />
				</div>
			</div>
