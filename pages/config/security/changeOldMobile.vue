<template>
	<view class="container">
		...
		<view class="lnk-common lnk-primary" @tap="submit">校验</view>
...
import { verifyMobile } from "../../../api/members"; //验证码模块
export default {
	name: '',
	components: {
		myVerification
	...
	watch: {
		async flage(val) {
			if (val) {
				...
				let res = await sendMobile(this.userInfo.mobile, "BIND_MOBILE")
		...
		submit() {
			if (!this.submitting) {
				...
				verifyMobile(params).then(res => {
					...
					if (res.data.result) {
						uni.navigateTo({
							url: "/pages/config/security/changeMobile",
						});
						setTimeout(() => {
							this.$u.toast("校验成功");
						}, 500)
