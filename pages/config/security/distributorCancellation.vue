        ...
        <view class="wrap-service">
            <view class="info">
                ...
                <view class="rs" v-if="!isSubmit">
                    <view class="txt" style="color:#FF0000">请先完成上面未完成事件在提交注销分平台服务商申请</view>
                </view>
            </view>
        </view>
...
export default {
    ...
    methods: {
        cancellation() {
            uni.showModal({
                title: '提示',
                content: '是否确认申请注销分平台服务商',
                success: (res) => {
                    if (res.confirm) {
                        applyCancelDisAndSup("2").then(res => {
    ...
    onLoad: function () {
        cancelCheck().then(res => {
            if (res.data.code == 200) {
                this.objCancel = res.data.result.selfDealer;
