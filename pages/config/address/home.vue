<template>
    <view class="container">
        <view class="wrap">
            <!-- 空数据 -->
            <view class="null" v-if="isComplete && arrData.length == 0"><empty-box text="您目前没有添加收件地址" /></view>
            <!-- 有数据 -->
            <view class="list" v-if="isComplete && arrData.length > 0">
                <view class="item" :class="{ item_hov: item.move }" v-for="(item, index) in arrData" :key="index"
                    ...
                    <view class="bd">
                        <address-item :index="index" :address="item" :mobile="item.mobile" @update="handlerUpdate"
                            @del="handlerDelete" @selAddress="handlerSelAddress(item)" :isSel="isSel" />
                    </view>
                    <view class="ft">
                ...
                <view class="maxNum" v-if="arrData.length >= 10">您的收货地址已达最大数量</view>
            </view>
        </view>
        <view class="fixed-link" v-if="arrData.length < 10">
            <view class="btn" @tap="handlerInsert">添加收件地址</view>
        </view>
    </view>
</template>
...
import { updateAddress } from '@/api/order.js'
export default {
    name: '',
    components: {
        AddressItem,
    ...
    methods: {
                ...
                success: (res) => {
                    if (res.confirm) {
                        if (!this.temp) {
                            let eCode = e.code
                            let eIndex = e.index
                            API_Address.deleteAddress(eCode).then((res) => {
        ...
        handlerUpdate: function (e) {
            if (this.orderId) {
                ...
                uni.navigateTo({
                    url: `/pages/config/address/update?data=` + JSON.stringify(this.arrData[e.index])
                })
            }
        },
        // 选择地址
        handlerSelAddress(item) {
            let pages = getCurrentPages() // 获取当前页面栈的实例，以数组形式按栈的顺序给出，第一个元素为首页，最后一个元素为当前页面。
            let prevPage = pages[pages.length - 2] //上一页页面实例
            uni.navigateBack({
                delta: 1,
                success: () => {
                    prevPage.$vm.loadAddressDetailFun(item.id) // 给上一页绑定方法otherFun,传参地址id
                    prevPage.$vm.loadAddressIdPath(item.consigneeAddressIdPath) // 给上一页绑定方法otherFun,传参地址id
                }
            })
        },
        ...
        handlerInsert: function () {
            uni.navigateTo({
                url: '/pages/config/address/insert'
            })
        },
        ...
        loadBaseData: function () {
            uni.showLoading()
            API_Address.getAddressList(this.params.pageNumber, this.params.pageSize).then((res) => {
    ...
    onLoad: function (options) {
        this.isSel = options.isSel ? JSON.parse(options.isSel) : false;
        if (options.id) {
