	...
	<view class="container">
		...
		<view class="wrap-state" v-if="isComplete">
			<view class="base">
				...
				<view class="txt">待重开</view>
			</view>
			<view class="tips">待重开发票金额：￥{{ objData.payPrice | unitPrice }}</view>
		</view>
		<view class="wrap-invoice">
			...
			<view class="bd">
				<view class="frm-comm">
					...
					<view class="rs rs-line">
						<view class="lab">冲红原因</view>
						<view class="txt">
							<view class="flex">
								<picker mode="selector" @change="handlerInvoice" :value="curRedInvoiceIndex"
									:range="arrRedInvoice" range-key="label">
									<view class="in-picker" :class="{ 'in-picker_place': curRedInvoiceLabel.length == 0 }">
										{{
											curRedInvoiceLabel.length == 0 ? '请选择冲红原因' : curRedInvoiceLabel }}</view>
								</picker>
							</view>
						</view>
					</view>
					<view class="rs rs-line" v-if="objData.needRedConfirm == 1">
						<view class="lab">红字确认单</view>
						<view class="txt">
							<view class="flex">
								<input class="s-text" v-model="objInvoice.redConfirmUuid" type="text"
									placeholder="请输入红字确认单" />
							</view>
						</view>
					</view>
				</view>
			...
			<view class="ft">
				<view class="lnk-common lnk-primary" @tap="handlerSubmit">提交换开</view>
			</view>
...
<script>
import { getInvoiceInfo, reopenInvoice, getCompanyInfo } from "@/api/invoice.js"
...
export default {
	...
	data() {
		return {
			...
			objInvoice: {
				...
				bankAccount: '',							// 开户银行
				bankAccountNo: '',					// 银行账户
				redConfirmUuid: '',                  //红字确认单
				redInvoiceLabel: '',                  //冲红原因
			},
	...
	methods: {
		...
		loadCompanyInfo() {
			...
			getCompanyInfo({ companyName: this.objInvoice.unitName }).then(res => {
				if (res.data.code == 200) {
					...
					if (this.companyList.length > 0 && this.objInvoice.unitName) {
		...
		handlerSubmit() {
			...
			if (!this.objInvoice.redInvoiceLabel) {
				this.$u.toast('请填写冲红原因')
				return
			}
			if (this.objData.needRedConfirm == 1) {
				if (!this.objInvoice.redConfirmUuid) {
					this.$u.toast('请填写红字确认单')
					return
				}
			}
			...
			reopenInvoice(this.objInvoice).then((res) => {
				if (res.data.success) {
					this.$u.toast('提交成功')
					uni.navigateBack()
				}
		...
		handlerInvoice: function (e) {
			this.curRedInvoiceIndex = e.detail.value
			this.curRedInvoiceLabel = this.arrRedInvoice[e.detail.value].label
			this.objInvoice.redInvoiceLabel = this.arrRedInvoice[e.detail.value].value
		},
