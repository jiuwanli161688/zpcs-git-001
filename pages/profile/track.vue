<template>
	<view class="container" :class="{'container-patch': isEditMode}">
		<view class="tips" v-if="isComplete && arrData.length > 0">
			<view class="text">为您展示最多100条历史足迹</view>
			<view class="link">
	...
	export default{
		...
		methods: {
					...
					if (res.statusCode == 200) {
						...
						if (data.length > 0) {
							data.forEach(item => {
								...
								if(!item.storeName && !item.goodsName && !item.price){
									item.marketEnable='DOWN'
								}else{
			...
			handlerDelete: function(e){
				...
				deleteHistoryListId(this.arrData[eIndex].goodsId).then((res) => {
					if (res.data.code == 200) {
						// 数据列表中商品ID
						let eId = this.arrData[eIndex].goodsId
						// 选中数组中，查找eId索引值
        ...
        this.isCanSubmit = false
        // 先执行接口删除
				deleteHistoryListId(ids).then((res) => {
					if (res.data.code == 200) {
						this.arrChecked.forEach(o => {
						let eIndex = this.arrData.findIndex(item => item.goodsId == o)
						if(eIndex >= 0){
							this.arrData.splice(eIndex, 1)
						}
					})
			...
			recalculateCheckedItems: function() {
				this.arrChecked = this.arrData.filter(item => item.isCheck).map(item => item.goodsId);
			},
			// 选中商品
