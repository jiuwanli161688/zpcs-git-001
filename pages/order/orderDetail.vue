<template>
  <view>
    ...
    <view class="info-view" v-if="order.deliveryMethod == 'SELF_PICK_UP'">
      <view class="address-view">
        <view>
          ...
          <view class="order-info-view" @tap="callPhone">
    ...
    <view>
      <view class="seller-view">
        ...
        <view class="seller-info u-flex u-row-between">
          <view class="seller-name" @tap="goToShopPage(order)">
        ...
        <view>
          <view v-for="(sku, skuIndex) in orderGoodsList" :key="skuIndex">
            <view class="goods-item-view">
              <view class="goods-img" @tap="gotoGoodsDetail(sku)">
                <u-image border-radius="6" width="131rpx" height="131rpx" :src="sku.image"></u-image>
              </view>
              <view class="goods-info" @tap="gotoGoodsDetail(sku)">
              ...
              <view class="goods-num">
                ...
                <view class="good-complaint">
                  <u-tag size="mini" mode="plain" @tap="complaint(sku)" v-if="sku.complainStatus == 'NO_APPLY'"
                    text="投诉" type="info" />
                </view>
              </view>
            </view>
      ...
      <view style="width: 100%">
        ...
        <view class="customer-list">
          <view class="customer-service"
            ...
            @tap="onCancel(order.sn)">取消订单</view>
          <view class="customer-service" v-if="order.orderStatus == 'DELIVERED'" @tap="onLogistics(order)">查看物流</view>
          <view class="customer-service" v-if="order.orderStatus != 'UNPAID' && order.orderPromotionType == 'PINTUAN'"
            @tap="ByUserMessage(order)">查看拼团信息</view>
          <view class="customer-service" @tap="contact(order.storeId)">联系客服</view>
        </view>
      </view>
    </view>
    <view class="info-view">
      <view style="width: 100%">
        <view class="order-info-view">
          ...
          <view class="value">
            {{ order.sn }}
            <u-tag class="copy" text="复制" type="info" size="mini" @tap="onCopy(order.sn)" />
          </view>
        </view>
        <view class="order-info-view">
    ...
    <view class="info-view" v-if="order.payStatus == 'PAID'">
      <view>
        <view class="invoice-info-view">
          ...
          <view v-else class="value" @tap="onReceipt(orderDetail.receipt)">查看发票</view>
        </view>
      </view>
    </view>
    ...
    <view class="bottom_view">
      <view class="btn-view u-flex u-row-between">
        ...
        <view>
          <!-- 全部 -->
          <!-- 等待付款 -->
          <u-button type="error" ripple size="mini"
            v-if="orderDetail.allowOperationVO && orderDetail.allowOperationVO.pay" @tap="toPay(order)">立即付款</u-button>
    ...
    <u-popup class="cancel-popup" v-model="cancelShow" mode="bottom" length="60%">
      ...
      <view class="footer">
        <u-button size="medium" v-if="reason" shape="circle" @tap="submitCancel">提交</u-button>
      </view>
    </u-popup>
    <u-toast ref="uToast" />
