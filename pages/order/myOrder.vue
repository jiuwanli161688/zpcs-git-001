<template>
  <view class="content">
      ...
      <swiper-item class="tab-content" v-for="(tabItem, tabIndex) in navList" :key="tabIndex">
        <scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData(tabIndex)">
          ...
          <view class="seller-view" :key="oderIndex" v-for="(order, oderIndex) in tabItem.orderList">
            ...
            <view>
              <view>
                <view class="goods-item-view" @tap="navigateToOrderDetail(order.sn)">
              ...
              <view class="btn-view u-flex u-row-between">
                ...
                <view class="goods-btn flex flex-a-c">
                  <!-- 全部 -->
                  <view ripple class="pay-btn" shape="circle" size="mini" v-if="order.allowOperationVO.pay"
                    @tap="waitPay(order)">立即付款</view>
                  <!-- 取消订单 -->
                  <view ripple class="cancel-btn" shape="circle" size="mini" v-if="order.allowOperationVO.cancel"
                    @tap="onCancel(order.sn)">
                    取消订单
                  </view>
                  <!-- 等待收货 -->
                  <view ripple shape="circle" class="rebuy-btn" size="mini" v-if="order.allowOperationVO.showLogistics"
                    @tap="navigateToLogistics(order)">
                    查看物流
                  </view>
                  <view ripple shape="circle" class="pay-btn" size="mini" v-if="order.allowOperationVO.rog"
                    @tap="onRog(order.sn)">
                    确认收货
                  </view>
    ...
    <u-popup class="cancel-popup" v-model="cancelShow" mode="bottom" length="60%">
      ...
      <view class="footer">
        <u-button size="medium" ripple v-if="reason" shape="circle" @tap="submitCancel">提交</u-button>
      </view>
    </u-popup>
    <u-toast ref="uToast" />
