  ...
  <view v-if="serviceDetail">
    ...
    serviceDetail.serviceStatus | serviceStatusList
      ...
      serviceDetail.serviceStatus | statusFilter
        ...
        <view class="log-box-top" @tap="onProgress()">
          <view class="top01">
            <view>审核日志</view>
    ...
    logs[0].message
      ...
      <view class="goods-info">
        <view class="info-box">
          <view class="goods-item-view" @tap="navgiateToGoodsDetail(serviceDetail)">
    ...
    serviceTypeList[serviceDetail.serviceType]
      ...
      <view class="detail-item" v-if="serviceDetail.afterSaleImage &&
    ...
    ">
        <view v-for="(img, index) in serviceDetail.afterSaleImage.split(',')" :key="index">
          <u-image width="100" height="100" :src="img"
            @tap="preview(serviceDetail.afterSaleImage.split(','), index)"></u-image>
        </view>
      </view>
      <!-- 如果服务类型为退款则不显示 -->
      <view class="detail-item"
    ...
    serviceDetail.accountType | accountTypeFilter
        ...
        <view class="detail-item" v-if="accountShow && !bankShow && serviceDetail.actualRefundPrice != 0
    ">
...
export default {
  ...
  methods: {
    ...
    onProgress() {
      uni.navigateTo({
        url: `./applyProgress?sn=${this.serviceDetail.sn
          }&createTime=${encodeURIComponent(this.serviceDetail.createTime)}
         &logs=${encodeURIComponent(JSON.stringify(this.logs))}&serviceStatus=${this.serviceDetail.serviceStatus
          }`,
      });
    },
  },
