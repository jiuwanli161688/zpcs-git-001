  ...
  <view>
    <view class="seller-view" v-for="(item, index) in complaionData" :key="index">
      ...
      <view class="goods-item-view">
        <view class="goods-img" @tap="handleToGoods(item)">
          <u-image border-radius="6" width="131rpx" height="131rpx" :src="item.goodsImage"></u-image>
        </view>
        <view class="goods-info" @tap="handleToGoods(item)">
      ...
      <view class="complain-btn">
        <u-tag mode="plain" @tap="handleClear(item)" class="complain-tag" text="撤销投诉" type="info" v-if="item.complainStatus != 'EXPIRED' && item.complainStatus != 'CANCEL'
      " />
        <u-tag mode="plain" @tap="handleInfo(item)" class="complain-tag" text="投诉详情" type="info" />
      </view>
    </view>

    <u-empty v-if="empty" :style="{ 'marginTop': complaionDetail.total == 0 ? '200rpx' : '0rpx' }" class="empty" style=""
      text="暂无投诉列表" mode="list"></u-empty>
