<template>
  <view class="edition-intro">
    ...
    <u-cell-group class="cell" :border="false">
      <!--  #ifdef APP-PLUS -->
      <u-cell-item v-if="IosWhether" @tap="checkStar" title="去评分"></u-cell-item>
      <u-cell-item title="功能介绍" @tap="navigateTo('/pages/mine/set/versionFunctionList')"></u-cell-item>
      <u-cell-item title="检查更新" @tap="checkUpdate"></u-cell-item>
      <!--  #endif -->

      <u-cell-item title="证照信息" @tap="navigateTo('/pages/mine/help/tips?type=LICENSE_INFORMATION')"></u-cell-item>
      <u-cell-item title="服务协议" @tap="navigateTo('/pages/mine/help/tips?type=USER_AGREEMENT')"></u-cell-item>
      <u-cell-item title="隐私协议" @tap="navigateTo('/pages/mine/help/tips?type=PRIVACY_POLICY')"></u-cell-item>
      <u-cell-item title="关于我们" :border-bottom="false"
        @tap="navigateTo('/pages/mine/help/tips?type=ABOUT')"></u-cell-item>

    </u-cell-group>

    <view class="intro">
      <view>{{ config.customerServiceMobile ? `客服热线：${config.customerServiceMobile}` : `` }}</view>
      <view style="margin:20rpx 0 0 0;">{{ config.customerServiceEmail ? `客服邮箱：${config.customerServiceEmail}` : `` }}
      </view>

      <view>
        <view style="margin:20rpx 0; color:#003a8c;" @tap="navigateTo('/pages/mine/help/tips?type=USER_AGREEMENT')">
          《{{ config.name }}用户协议》</view>
