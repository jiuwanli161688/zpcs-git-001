  ...
  <view class="address">
    ...
    <view class="list" v-else>
      <view class="item c-content" v-for="(item, index) in addressList" :key="index">
        <view class="basic" @tap="selectAddressData(item)">
          <text>{{ item.name }}</text>
        ...
        <view class="edit">
          <view class="relative" @tap="setDefault(item)">
          ...
          <view class="relative">
            ...
            <text class="mr-40" @tap="addAddress(item.id)">编辑</text>
            <view class="alifont icon-lajitong"></view>
            <text @tap="removeAddress(item.id)">删除</text>
          </view>
        </view>
      </view>
      <view style="height: 100px"></view>
    </view>
    <button type="default" class="btn" @tap="addAddress('')">
      <u-icon name="plus-circle"></u-icon>
      添加新收货人
    </button>
    <u-action-sheet :list="removeList" :tips="tips" v-model="showAction" @tap="deleteAddressMessage"></u-action-sheet>
  </view>
</template>
