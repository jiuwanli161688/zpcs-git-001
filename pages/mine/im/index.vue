  ...
  <view class="wrapper">
              ...
              <view class="margin-left padding-chat bg-user-orang" style="border-radius: 35rpx; ">
                ...
                <view v-if="item.messageType == 'GOODS'">
                  <view class="goods-card u-flex u-row-between u-p-b-0" style="width:100%;margin: 0 0; ">
                    <view class="image-box" @tap="jumpGoodDesc(item)">
                      <image class="image" :src="JSON.parse(item.text)['thumbnail']" mode="widthFix"></image>
                    </view>
                    <view class="goods-desc" @tap="jumpGoodDesc(item)">
                ...
                <view v-if="item.messageType == 'ORDER'" @tap="linkTosOrders(item.text)">
                  <view class="order-sn">
                ...
                <view v-if="item.messageType === 'ORDER'">
                  <view class="order-sn">
                    ...
                    <view class="order-list">
                      ...
                      <view class="name-or-time">
                        <text @tap="linkTosOrders(item.text)">{{
    ...
    <view :style="{ position: 'fixed', bottom: inputHeight + 'px' }" class="flex-column-center"
      ...
      <view class="bottom-dh-char flex-row-around" style="font-size: 55rpx;">
        <!-- vue无法使用软键盘"发送" -->
        <input @focus="inputBindFocus" @blur="eventHandle" :adjust-position="false" v-model="msg" class="dh-input"
          type="text" style="background-color: #f0f0f0;" @confirm="sendMessage" confirm-type="send"
          placeholder="用一句简短的话描述您的问题" />
        <view @tap="sendMessage" class="cu-tag bg-main-color send round">
          发送
        </view>
        <!-- <text @tap="ckAdd" class="cuIcon-roundaddfill text-brown"></text> -->
      </view>
    </view>
  </view>
</template>
