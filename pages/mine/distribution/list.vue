<template>
  <view class="wrapper">
    ...
    <!-- 导航栏 -->
    <view class="nav">
      <view class="nav-item" @tap="handleMyGoods(true)" :class="{ checked: params.checked }">已选择</view>
      <view class="nav-item" @tap="handleMyGoods(false)" :class="{ checked: !params.checked }">未选择</view>

      <!-- <view class="nav-item" @tap="popup = !popup">筛选</view> -->
    </view>
    <!-- 商品列表 -->

    <view class="goods-list">
      <u-swipe-action v-for="(item, index) in goodsList" :disabled="!params.checked" :show="item.___selected"
        @open="openAction(item)" :index="index" :options="options" bg-color="#fff" ref="swiperAction" :key="item.id"
        @tap="changeActionTab(item)">

        <div class="goods-item">
          <view class="goods-item-img" @tap="handleNavgationGoods(item)">
            <u-image width="176rpx" height="176rpx" :src="item.thumbnail"></u-image>
          </view>
          <view class="goods-item-desc">
            <!-- 商品描述 -->
            <view class="-item-title" @tap="handleNavgationGoods(item)">
              {{ item.goodsName }}
            </view>
            <!-- 商品金额 -->
            <view class="-item-price" @tap="handleNavgationGoods(item)">
              佣金:
              <span> ￥{{ item.commission | unitPrice }}</span>
            </view>
            <!-- 比率佣金 -->
            <view class="-item-bottom">
              <view class="-item-bootom-money" @tap="handleNavgationGoods(item)">
                <!-- <view class="-item-bl">
                比率:
                <span>{{ "5.00%" }}</span>
              </view> -->
              ...
              <view>
                <view class="click" v-if="!params.checked" @tap="handleClickGoods(item)">立即选取</view>
                <view class="click" v-if="params.checked" @tap="handleLink(item)">分销商品</view>
              </view>
            </view>
          </view>
        </div>
