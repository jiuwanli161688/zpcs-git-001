<template>
	<view class="content">
			...
			<scroll-view class="keyword-list-box" v-show="isShowKeywordList" scroll-y>
				<block v-for="(row, index) in keywordList" :key="index">
					<view class="keyword-entry" hover-class="keyword-entry-tap">
						<view class="keyword-text" @tap.stop="doSearch(keywordList[index].words)"><rich-text
								:nodes="row.words"></rich-text></view>
					</view>
				</block>
			</scroll-view>
			<div class="keyword-box" v-show="!isShowKeywordList">
				...
				<view class="keyword-block" v-if="oldKeywordList.length > 0">
					...
					<div class="oldKeyList">
						...
						<div @tap="showMore" v-if="oldKeywordIndex > loadIndex" class="oldKeyItem">展示更多</div>
					</div>
				</view>
		...
		<view class="goods-content" v-if="isShowSeachGoods">
			<view class="navbar">
				<view class="nav-item" :class="{ current: filterIndex === 0 }" @tap="tabClick(0)">综合排序</view>
				<view class="nav-item" :class="{ current: filterIndex === 3 }" @tap="tabClick(3, 'buyCount')">
					<text>销量</text>
					<view class="p-box">
						<view class="index-nav-arrow">
							<image class="img" :src="arrowUpAtv" v-if="params.sort === 'buyCount' && params.order === 'asc'"
				...
				<view class="nav-item" :class="{ current: filterIndex === 4 }" @tap="tabClick(4, 'price')">
					<text>价格</text>
					<view class="p-box">
						<view class="index-nav-arrow">
							<image class="img" :src="arrowUpAtv" v-if="params.sort === 'price' && params.order === 'asc'"
								mode="aspectFit"></image>
							<image class="img" :src="arrowUp" v-else mode="aspectFit"></image>
						</view>
						<view class="index-nav-arrow">
							<image class="img" :src="arrowDownAtv" v-if="params.sort === 'price' && params.order === 'desc'"
								mode="aspectFit"></image>
							<image class="img" :src="arrowDown" v-else mode="aspectFit"></image>
						</view>
					</view>
				</view>
				<view class="nav-item" @tap="sortGoods">筛选</view>
			</view>
			<!-- 一行一个商品展示 -->
			<div v-if="isSWitch">
		...
		<u-popup border-radius="20" mode="right" width="90%" v-model="sortPopup">
			...
			<view class="sort-box  ">
				<view class="sort-list">
					<!-- <view class="sort-item">
						...
						<view class="flex" v-if="sortData.brands">
							<view class="sort-brand-item" :key="brandsIndex" v-for="(brand, brandsIndex) in sortData.brands" @tap="handleSort(brand, brandsIndex, 'brand')">
								<view
									class="sort-radius"
									:class="{
					...
					<view class="sort-item">
						...
						<view class="flex" style="flex-wrap: wrap;" v-if="sortData.categories">
							<view class="sort-brand-item" :key="categoriesIndex"
								...
								@tap="handleSort(categoryId, categoriesIndex, 'categoryId')">
				...
				<view class="sort-list">
					<view class="sort-item">
						<view class="sort-title">中品严选</view>
						<view class="flex" style="flex-wrap: wrap;">
							<view class="sort-brand-item" @tap="handleSelfOperated(params.selfOperated)">
								<view class="sort-radius" :class="{
									'sort-active': params.selfOperated
								}">
									中品严选
								</view>
							</view>
					...
					<view class="sort-item">
						...
						<view style="display:flex;  margin-top:20rpx;    align-items: center;">
							<view class="sort-brand-item uinput">
								<view class="sort-radius"><u-input v-model="minPrice" type="number" placeholder="最低价"
										input-align="center" /></view>
							</view>
							<view>-</view>
							<view class="sort-brand-item uinput">
								<view class="sort-radius"><u-input v-model="maxPrice" type="number" placeholder="最高价"
										input-align="center" /></view>
							</view>
						</view>
					</view>
					...
					<view class="sort-item" :key="paramIndex" v-for="(param, paramIndex) in sortData.paramOptions">
						...
						<view class="flex" style="flex-warp:warp" v-if="param.values">
							<view class="sort-brand-item" :key="i" v-for="(value, i) in param.values"
								@tap="handleSort(value, i, 'prop', param)">
								<view class="sort-radius" :class="{
				...
				<view class="sort-btn mp-iphonex-bottom">
					<view class="sort-btn-repick" @tap="repick">重置</view>
					<view class="sort-btn-confim" @tap="sortConfim">确定</view>
				</view>
			</view>
		</u-popup>
...
export default {
	...
	onLoad(val) {
		...
		if(val.regionFlag){
			this.params.regionFlag = JSON.parse(val.regionFlag)
		}
		//  this.initSortGoods();
		// 接收分类的数据

		this.routerVal = val;
		this.params.pageSize = uni.getStorageSync("pageConfig")?.goodListSize || 10;
		...
		if (this.routerVal.keyword) {
			...
			this.defaultKeyword = this.routerVal.keyword;
			this.keyword = this.routerVal.keyword;
			this.isShowSeachGoods = true;
		}
	...
	methods: {
		...
		repick() {
		  this.keyword = '';
		  // 确保搜索框组件也清空
			if (this.$refs.mSearch) {
				this.$refs.mSearch.inputVal = '';
			}
			this.sortParams = {
				pageNumber: 1,
				pageSize: uni.getStorageSync("pageConfig")?.goodListSize || 10,
		...
		// 点击自营筛选
		handleSelfOperated(params) {
			this.params.selfOperated = params == '' ? true : '';
		},
		init() {
			// 加载搜索记录
		...
		navigateToDetailPage(item) {
			uni.navigateTo({
				url: `/pages/product/goods?id=${item.id}&goodsId=${item.goodsId}&source=SEARCH`
			});
		},
		// 跳转地址
		...
		initSortGoods() {
      this.sortParams = {}
      this.sortParams.keyword = this.keyword
      ...
      this.params = {selfOperated: '',regionFlag: this.params.regionFlag}
