...
export default {
  ...
  // 设置中品一号用户信息
  setZpMemberInfo(val) {
    uni.setStorageSync('zpMemberInfo', val);
  },
  // 是否发送商品连接记录
  getZpMemberInfo() {
    return uni.getStorageSync('zpMemberInfo');
  },
  // 是否发送商品连接记录
  removeZpMemberInfo() {
    uni.removeStorageSync('zpMemberInfo');
  },
  getImGoodsLink() {
  ...
  // 下单成功保存订单信息
  setOrderInfo(val) {
    uni.setStorageSync('orderInfo', val);
  },
  // 查询订单信息
  getOrderInfo() {
    return uni.getStorageSync('orderInfo');
  },
};
