...
function seTClientTypeEve() {
	let clientType = "UNKNOWN"
	// #ifdef MP-WEIXIN
		clientType = "WEIXIN_MP"
	// #endif
	// #ifdef APP
		clientType = "APP"
	// #endif
...
http.interceptors.response.use(
	async (response) => {
		...
		//   isRefreshing = false;
		// }
		// 去掉切换页面的 loading 闪现闪屏
		let tabbarPageRoute=['pages/tabbar/home/index','pages/tabbar/category/category','pages/tabbar/cart/cartList','pages/tabbar/user/profile','pages/product/goods']
		if(tabbarPageRoute.includes(getCurrentPages()[getCurrentPages().length - 1]?.route)){
			//由于关联页面太多，为了体验现控制tabbar 的四个页面的loading闪屏，
		}else{
			//其他页面还是得开启，由于内页好多 loading，必须得关一下
