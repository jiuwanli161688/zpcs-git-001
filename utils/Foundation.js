 ...
 * @param source  1 || 2  //来源，1为登录来源，2为正常拦截 正常拦截只做拦截相关的功能，不进行其他跳转。
 */

export function whetherNavigate(type = "default",source) {

  let userInfo = storage.getUserInfo()
  if (userInfo && userInfo !== '' && Object.keys(userInfo).length > 0 && userInfo.mobile == null) {
    uni.redirectTo({
      url: "/pages/login/bind",
    })
    return
  }
  if (userInfo && userInfo !== '' && Object.keys(userInfo).length > 0 && userInfo.bindDistributorId == null) {
    uni.reLaunch({
      url: "/pages/login/shop",
    })
    return
  }
  //如果是登录来源，那就跳转，否则不执行下面的跳转逻辑。
  if (source == 1){
...
export function tampToDate(tamp, formatStr = 'Y-M-D h:m:s') {
  var date = new Date(tamp * 1000);				//时间戳为10位需*1000，时间戳为13位的话不需乘1000
