 ...
 */
 export function getGoods(skuId, goodsId, source) {
  return http.request({
    url: `/goods/goods/sku/${goodsId}/${skuId}?source=${source}`,
    method: Method.GET,
  });
}
...
/**
 * 获取当前用户的分销商信息 可根据分销商信息查询待提现金额以及冻结金额等信息
 */
 ...
 */
 export function getFirstCategory(){
  return http.request({
    url:`/goods/category/firstCategory`,
    method:Method.GET,
 ...
 * 根据分类获取资质列表
 */
 export function getCategoryCompetency(params){
  return http.request({
    url:`/goods/category/getCompetency`,
    method:Method.GET,
    params
  })
}

export function getPopularCategoryTree() {
  return http.request({
